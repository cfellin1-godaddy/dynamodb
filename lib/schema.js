...
internals.parseDynamoTypes = function (data) {
  if(_.isPlainObject(data) && data.type === 'object' && _.isPlainObject(data.keys)) {
    return internals.parseDynamoTypes(data.keys);
  }

  var mapped = _.reduce(data, function(result, val, key) {
    if(val.type === 'object' && _.isPlainObject(val.keys)) {
      result[key] = internals.parseDynamoTypes(val.keys);
    } else if (val.type === 'array' && _.isArray(val.items) && val.items.length && (val.metas === undefined || val.metas.length < 1)) {
